...
{spec에서 Core 섹션 — What / Benefit 1-2줄 요약}

## 주요 스펙

### 접근 권한
{spec 2.1 접근 권한 테이블 요약}

### 입력 검증
{spec 2.2 입력 데이터 검증 요약}

### 비즈니스 규칙
{spec 2.3 비즈니스 규칙 목록}

## 오류 처리
{spec 4. 오류 처리 — 검증/비즈니스/시스템 오류 요약}
...
**스크립트 구조 (`/tmp/clickup-capture-{도메인}.mjs`):**
...
const BASE = "http://localhost:{확인된 포트}/ko";  // 포트 동적 반영
const SCREENSHOT_DIR = "/tmp/clickup-screenshots";
const wait = (ms) => new Promise((r) => setTimeout(r, ms));

mkdirSync(SCREENSHOT_DIR, { recursive: true });

const captures = [
  { name: "account-personal-info", url: "/account" },
  { name: "account-security", url: "/account", action: "click-tab:로그인 및 보안" },
  // ...캡처 대상 목록
];
...
(async () => {
  ...
  for (const cap of captures) {
    await page.goto(`${BASE}${cap.url}`, { waitUntil: "networkidle0", timeout: 30000 });
    await wait(3000);

    // 탭 클릭 등 추가 액션
    if (cap.action?.startsWith("click-tab:")) {
      const tabText = cap.action.replace("click-tab:", "");
    ...
    await page.screenshot({
      path: path.join(SCREENSHOT_DIR, `${cap.name}.png`),
      fullPage: false,
    });
    console.log(`캡처 완료: ${cap.name}`);
  }
...
```bash
node /tmp/clickup-capture-{도메인}.mjs
```
...
**예시:**
```
pw-01-로그인-및-보안-탭.png
pw-02-비밀번호-변경-모달.png
pw-03-비밀번호-요구사항-미충족.png
pw-04-비밀번호-불일치.png
pw-05-비밀번호-입력-완료.png

email-01-개인정보-탭.png
email-02-이메일-변경-모달.png
email-03-새-이메일-입력.png
email-04-인증코드-입력-화면.png

withdraw-01-탈퇴-신청-버튼.png
withdraw-02-탈퇴-절차-안내.png
withdraw-03-삭제-정보-확인.png
withdraw-04-약관-확인-및-동의.png
withdraw-05-약관-스크롤-완료.png
```
